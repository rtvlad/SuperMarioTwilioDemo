...
public class PlayScreen implements Screen{
    ...
    public void handleIndirectInput(int key) {
        if(player.currentState == Mario.State.DEAD) {
            //ressurect Mario
            game.toBeRestarted = true;
        }

        //control our player using immediate impulses
        if (key == Input.Keys.UP)
            player.jump();
        if (key == Input.Keys.RIGHT)// && player.b2body.getLinearVelocity().x <= 2)
            player.b2body.applyLinearImpulse(new Vector2(1.0f, 0), player.b2body.getWorldCenter(), true);
        if (key == Input.Keys.LEFT)// && player.b2body.getLinearVelocity().x >= -2)
            player.b2body.applyLinearImpulse(new Vector2(-1.0f, 0), player.b2body.getWorldCenter(), true);
